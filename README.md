<!-- Animated Typing Header -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Inter&size=28&duration=3000&pause=1000&color=F5F5F5&center=true&vCenter=true&width=900&lines=Kevin+Lin;Senior+Software+Engineer;Distributed+Systems+%7C+Cloud+Infrastructure;Designing+Reliable+Systems+at+Scale" />
</p>

<p align="center">
Senior Software Engineer | Distributed Systems | Cloud & Compute Infrastructure
</p>

---

<p>
Senior Software Engineer with 8+ years of experience designing and scaling production-grade backend platforms and cloud-native systems. I specialize in distributed system architecture, multi-tenant infrastructure, and high-throughput services built with TypeScript and Go.

I focus on reliability, workload isolation, performance optimization, and full lifecycle ownership — from system design and implementation to deployment, observability, and operational excellence.
</p>

---

## 🏗 Architecture Highlights

- Designed and operated **high-throughput backend services** powering financial workflows in production fintech systems.
- Contributed to **federated, multi-tenant compute platform infrastructure** supporting streaming, batch, and ML workloads.
- Built **cloud-native services on Kubernetes** with strong reliability and isolation guarantees.
- Improved system performance and stability through **scalability optimization, monitoring instrumentation, and service hardening**.
- Delivered internal platform abstractions enabling engineering teams to ship faster and more safely.

---

## 🛠 Technical Stack

### 💻 Languages
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="42" title="TypeScript"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg" height="42" title="Go"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="42" title="Python"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="42" title="JavaScript"/>
</p>

### ☁️ Cloud & Infrastructure
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" height="42" title="AWS"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/googlecloud/googlecloud-original.svg" height="42" title="GCP"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="42" title="Docker"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" height="42" title="Kubernetes"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apachekafka/apachekafka-original.svg" height="42" title="Apache Kafka"/>
</p>

### 🗄 Data & Observability
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="42" title="PostgreSQL"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg" height="42" title="Redis"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/prometheus/prometheus-original.svg" height="42" title="Prometheus"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/grafana/grafana-original.svg" height="42" title="Grafana"/>
</p>

---

## 📫 Connect With Me

<p align="center">
  <a href="mailto:kl951210@outlook.com">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/google/google-original.svg" height="42" title="Email Me"/>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://www.linkedin.com/in/klin1344/">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" height="42" title="LinkedIn Profile"/>
  </a>
</p>
