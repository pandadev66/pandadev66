<h1 align="left">Hi, I am Kevin 👋</h1>

<p>
Senior Software Engineer | Distributed Systems | Cloud & Compute Infrastructure
</p>

<p>
Senior Software Engineer with 8+ years of experience designing and scaling production-grade backend platforms and cloud-native systems. I specialize in TypeScript and Go-based services, distributed system architecture, and multi-tenant infrastructure operating at scale.

I focus on reliability, workload isolation, performance optimization, and owning systems end-to-end — from architecture and implementation to deployment, observability, and operational excellence.
</p>

---

## 🔹 Core Expertise

- Designing and operating **distributed systems at production scale**
- Building **multi-tenant, cloud-native services** with strong reliability guarantees
- Architecting **high-throughput APIs and backend platforms**
- Containerization & orchestration with **Docker & Kubernetes**
- Deep experience in **AWS & GCP infrastructure**
- Performance optimization, service hardening, and operational readiness
- End-to-end ownership: **design → implementation → deployment → support**

---

## 🛠 Technical Stack

### 💻 Languages
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="42" title="TypeScript"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg" height="42" title="Go"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="42" title="Python"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="42" title="JavaScript"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="42" title="SQL"/>
</p>

### ☁️ Cloud & Infrastructure
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" height="42" title="Amazon Web Services (AWS)"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/googlecloud/googlecloud-original.svg" height="42" title="Google Cloud Platform (GCP)"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="42" title="Docker"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" height="42" title="Kubernetes"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apachekafka/apachekafka-original.svg" height="42" title="Apache Kafka"/>
</p>

### 🗄 Data & Storage
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="42" title="PostgreSQL"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" height="42" title="MongoDB"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg" height="42" title="Redis"/>
</p>

### 📊 Observability
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/prometheus/prometheus-original.svg" height="42" title="Prometheus"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/grafana/grafana-original.svg" height="42" title="Grafana"/>&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="42" title="GitHub"/>
</p>

---

## 🌱 Current Focus

- Large-scale compute platform design  
- Reliability engineering & workload isolation  
- Infrastructure-level performance optimization  
- Platform abstractions for internal developer velocity  

---

## 📫 Connect With Me

<p>
  <a href="mailto:kl951210@outlook.com">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/google/google-original.svg" height="42" title="Email Me"/>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://www.linkedin.com/in/klin1344/">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" height="42" title="LinkedIn Profile"/>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/pandadev66">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="42" title="GitHub Profile"/>
  </a>
</p>
